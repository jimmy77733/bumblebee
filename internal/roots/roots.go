...
func Resolve(profile string, explicit []string, opts Options) (roots []scanner.Root, notes []string, err error) {
	switch profile {
...
func Classify(path, profile string) string {
...
func IsDriveRoot(path string) bool {
	...
	if driveRootCleaned(filepath.Clean(path)) {
		return true
	}
	abs, err := filepath.Abs(path)
	if err != nil {
	...
	return driveRootCleaned(filepath.Clean(abs))
}

func driveRootCleaned(abs string) bool {
	...
	case "windows":
		...
		if userProfile := os.Getenv("USERPROFILE"); userProfile != "" {
			roots = append(roots, scanner.Root{
				Path: filepath.Join(userProfile, ".nuget", "packages"),
...
func baselineDefaultRoots(opts Options) ([]scanner.Root, []string) {
...
func projectDefaultRoots(opts Options) ([]scanner.Root, []string) {
	var candidates []scanner.Root
	for _, home := range homesForExpansion(opts) {
...
func homesForExpansion(opts Options) []string {
	if opts.AllUsers && runtime.GOOS == "darwin" {
